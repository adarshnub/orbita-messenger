...
async function loadContactNicknames(supabase: SupabaseClient, userId: string) {
  ...
  return new Map(
    (data ?? [])
      .map((row) => [String(row.contact_user_id), typeof row.nickname === "string" ? row.nickname.trim() : ""])
...
async function loadContacts(supabase: SupabaseClient, userId: string) {
  ...
  return (data ?? []).map((row) => {
    ...
    return mapProfile({ ...profileRow, nickname: row.nickname }, userId);
  });
}
...
async function loadConversations(supabase: SupabaseClient, userId: string) {
  ...
  const contactNicknames = await loadContactNicknames(supabase, userId);
  const loaded = await Promise.all(
    (conversations ?? []).map(async (conversation) => {
      ...
      const mappedParticipants = (participants ?? []).map((row) => {
        const profileRow = (row.profiles ?? {}) as Record<string, unknown>;
        return {
          ...mapProfile({ ...profileRow, nickname: contactNicknames.get(String(profileRow.id ?? "")) }, userId),
          role: row.role,
        };
      });
...
async function forwardTaskmanagerInbound(
  ...
  if (!activeLink) {
    ...
    const participants = await loadConversationParticipants(supabase, conversationId);
    const senderParticipant = participants.find((row) => String((row as Record<string, unknown>).user_id ?? "") === senderId);
    ...
    const agentParticipant = participants.find((row) => {
      ...
      const about = typeof profile?.about === "string" ? profile.about.trim().toLowerCase() : "";
      return about === "task manager agent";
    });
    const agentParticipantId = String((agentParticipant as Record<string, unknown> | undefined)?.user_id ?? "");
    ...
    const { data: fallbackLink, error: fallbackError } = await supabase
      ...
      .eq("agent_profile_id", agentParticipantId)
      .eq("enabled", true)
      .order("updated_at", { ascending: false })
      .limit(1)
      .maybeSingle();
...
async function handleAction(supabase: SupabaseClient, user: User, action: string, payload: Record<string, unknown>) {
  ...
  if (action === "add_contact_by_phone") {
    ...
    const contactRow: { owner_id: string; contact_user_id: unknown; nickname?: string | null } = {
      owner_id: user.id,
